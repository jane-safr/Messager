...
function checkChat()
{
  ...
  if( elements[0] && (<HTMLInputElement>elements[0]).checked)
  {
  ws.send(
    JSON.stringify({
    sys: 'Yes',
 ...
 function checkAll()
{
     ...
     (<HTMLInputElement>document.getElementById("addChat")).value='';
     ws.send(
      JSON.stringify({
      sys: 'Yes',
      act: 'checkUsersInChat',
...
let ind;
 function History1(message,userId)
 {
  message.History.forEach(message => {
    ...
    let li1: HTMLLIElement;
      if ( userId != message.idUserFrom)
            ...
            li1.style.fontStyle = "italic;right";
            li1.style.color = "rgb(150, 114, 73)";
      ...
      function  checkUsersInChat(message)
      {
       ...
       (<HTMLInputElement>document.getElementById("addChat")).value =  (<HTMLInputElement>document.getElementById("ФИО" + message.idChat)).value;
...
let clName = x=="Contacts"? "skryt":"vid";

document.getElementById("input").className = clName;
 let p = document.getElementsByTagName("p");
let i;
for (i = 0; i < p.length; i++) {
  p[i].className = clName;
}
}
